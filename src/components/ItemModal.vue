<template>
  <div class="fixed top-0 left-0 right-0 bottom-0 bg-gray-600 bg-opacity-80 z-30">
    <div class="absolute bg-gray-100 top-14 left-2/4 transform -translate-x-1/2
      border border-gray-600 p-1 shadow-lg min-w-sm" style="height:500px">
      <div class="relative border-8 border-gray-600 h-full p-4">
        <h3 class="text-3xl text-center pt-3 pb-5">項目詳情</h3>
        <div class="flex mb-5">
          <label for="title" class="text-xl mr-3">標題</label>
        ...
        <div class="flex mb-5">
          <label for="description" class="text-xl mr-3">描述</label>
        ...
        <div class="flex items-center mb-5" v-if="!index && !(index === 0)">
          <label for="state" class="text-xl mr-3">狀態</label>
        ...
        <div class="flex">
          <label for="due_date" class="text-xl mr-3">期限日</label>
