  ...
  <div class="border-2 border-gray-600 border-opacity-40 relative transition-shadow duration-500 hover:shadow-md" :class="changeColor">
    <div class="group absolute top-2/4 left-0 transform -translate-x-1/2 -translate-y-1/2" v-if="!(state === 'todo')" style="width:1.875rem;height:1.875rem">
      ...
      <div class="relative rounded-full bg-gray-500 w-full h-full group-hover:bg-primary">
        <font-awesome-icon icon="chevron-circle-right" rotation="180" @click="$emit('last-list')" class="text-3xl text-gray-200"/>
      </div>
    </div>
    <div class="group absolute top-2/4 right-0 transform translate-x-1/2 -translate-y-1/2" v-if="!(state === 'done')" style="width:1.875rem;height:1.875rem">
      <div class="absolute rounded-full bg-primary w-full h-full opacity-50 group-hover:animate-ping"></div>
      <div class="relative rounded-full bg-gray-500 w-full h-full group-hover:bg-primary">
        <font-awesome-icon icon="chevron-circle-right" @click="$emit('next-list')" class="text-3xl text-gray-200"/>
      </div>
    </div>
    <div class="group mx-8">
      ...
      <div class="flex border-t-2 border-gray-600 border-opacity-40 mb-1">
        <button class="flex-1 p-1" @click="$emit('show-modal')"><font-awesome-icon icon="list-ul" class="text-lg text-gray-500 group-hover:text-primary"/></button>
